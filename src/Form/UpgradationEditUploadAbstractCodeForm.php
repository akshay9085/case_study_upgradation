...
class UpgradationEditUploadAbstractCodeForm extends FormBase {

  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'upgradation_edit_upload_abstract_code_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $proposal_id = NULL) {
    ...
    if (!$proposal) {
      ...
      return $this->buildRedirectNotice(
        ...
        'upgradation.proposal_edit_file_all',
        $this->t('Back to editable uploads')
      );
    }
    ...
    $form['cancel'] = [
      '#type' => 'item',
      '#markup' => Link::fromTextAndUrl($this->t('Cancel'), Url::fromRoute('upgradation.proposal_edit_file_all'))->toString(),
    ];
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    if (empty($_FILES['files'])) {
      $form_state->setErrorByName('upload_an_abstract', $this->t('Please upload at least one file.'));
      return;
    }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if (!$proposal) {
      $this->messenger()->addError($this->t('Invalid proposal selected. Please try again.'));
      $form_state->setRedirect('upgradation.proposal_edit_file_all');
      return;
    }
    ...
    foreach ($field_map as $field_name => $file_type) {
      ...
      if (!move_uploaded_file($_FILES['files']['tmp_name'][$field_name], $destination)) {
        $this->messenger()->addError($this->t('@file was not updated successfully.', ['@file' => $file_name]));
        continue;
      }
    ...
    $params['abstract_edit_file_uploaded']['proposal_id'] = $proposal->id;
    $params['abstract_edit_file_uploaded']['user_id'] = (int) $this->currentUser()->id();
    $params['abstract_edit_file_uploaded']['updated_files'] = $updated_files;
    ...
    $form_state->setRedirect('upgradation.edit_upload_abstract_code_form', ['proposal_id' => $proposal->id]);
  }
